-- ─────────────────────────────────────────────────────────────────────────────
-- SPAWN Clinical — Neon Postgres Schema (SINGLE-COMMAND variant)
--
-- Identical to db/schema.sql, but wrapped in one DO $$ … $$; block so the whole
-- thing is sent to Postgres as a SINGLE command. Use this when your runner uses
-- prepared/extended-query protocol (Vercel's inline Neon box, the serverless
-- HTTP driver, etc.) and rejects multi-statement scripts with:
--   "cannot insert multiple commands into a prepared statement"
--
-- Paste-run anywhere. Idempotent (IF NOT EXISTS throughout).
-- ─────────────────────────────────────────────────────────────────────────────
